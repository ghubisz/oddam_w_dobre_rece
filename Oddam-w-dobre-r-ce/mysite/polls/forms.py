    ...
    class Meta:
        ...
        error_messages = {
            'email': {
                ...
                'required': _('Adres e-mail jest wymagany.')
            }
        }
...
class UserPasswordChangeForm(PasswordChangeForm):
    ...
    new_password2 = forms.CharField(required=True,
                                    widget=forms.PasswordInput(),
                                    error_messages={
                                        'required': 'Potwierdzenie nowe hasła nie może być puste!'
...
class AddDonationStepFour(forms.Form):
    ...
    def clean_phone_number(self):
        ...
        if not phonenumbers.is_valid_number(phonenumbers.parse(phone_number,"PL")):
            raise forms.ValidationError(_("Niepoprawny number telefonu"))
        return phone_number
